...
function Navbar() {
  ...
  return (
    <>
      <nav className={`${styles.navbar} w-full fixed top-0 z-10`} style={{ zIndex: 100, }} >
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center"> {/* Добавленный общий контейнер */}
              ...
              <Link to="/" style={{ fontFamily: 'ExcentraPro-Bold', fontSize: '30px', paddingTop: '10px', marginLeft: '-5px', color: 'rgb(253, 144, 10)' }} className="ml-2 text-white font-bold text-lg"> {/* Добавленный отступ */}
                Wild Tours
              </Link>
            </div>
            <div className="flex space-x-9">
              ...
              {(!user || isUserLoading ) && (
                <>
                  <button onClick={() => openModal('registration')} style={{ fontFamily: 'ExcentraPro-Bold', fontSize: '25px', paddingTop: '5px', marginLeft: '25px' }} className="text-white">
                    Регистрация
                  </button>
                  <button onClick={() => openModal('login')} style={{ fontFamily: 'ExcentraPro-Bold', fontSize: '25px', paddingTop: '5px', marginLeft: '25px' }} className="text-white">
                    Вход
                  </button>
                </>
              )}
      ...
      {isModalOpen && (
        <div className={`fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-20 transition-opacity duration-300 ${isModalVisible ? 'opacity-100' : 'opacity-0'}`} onClick={handleOutsideClick}>
          <div
            className={`w-1/3 p-6 rounded-lg shadow-lg relative transition-transform duration-300 ${isModalVisible ? 'translate-y-0' : '-translate-y-10'}`}
            style={{
              top: '50px',
              right: '-550px',
          ...
          >
            ...
            <h2 className="text-lg font-bold">{modalType === 'registration' ? 'Регистрация' : modalType === 'logout' ? 'Выход' : 'Вход'}</h2>
